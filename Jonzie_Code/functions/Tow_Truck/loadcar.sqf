...
if ((getPos _truck) distance (getPos _towing) > 15) exitWith {titleText ["No car found, Too far away from vehicle", "PLAIN DOWN"];};
if ((_cardir + 20) <= _truckdir) exitWith  {titleText ["Vehicle not lined up properly", "PLAIN DOWN"];};
if ((_cardir - 20) >= _truckdir) exitWith  {titleText ["Vehicle not lined up properly", "PLAIN DOWN"];};
if ((_truck animationPhase "slide") != -4.5) exitWith {titleText ["Lower the ramp to the ground", "PLAIN DOWN"];};
if ((_towpoint distance _towing) <= 2) exitWith {titleText ["Too far away to tow", "PLAIN DOWN"];};
if(not (_towing isKindOf "Car_F")) exitWith {titleText ["Vehicle not supported", "PLAIN DOWN"];}; 
if (_truck == _towing) exitWith {titleText ["No car found, Too far away from vehicle", "PLAIN DOWN"];};
...
{
	_height = _height + 0.2;_distance = _distance -5.5;
	titleText ["Vehicle not officially supported", "PLAIN DOWN"];
	_UnSupported_Vehicle_Height = getNumber (configFile >> "CfgVehicles" >> Typeof _towing >> "Load_Height");_height = _UnSupported_Vehicle_Height;
	_UnSupported_Vehicle_Distance = getNumber (configFile >> "CfgVehicles" >> Typeof _towing >> "Load_Distance");_distance = _UnSupported_Vehicle_Distance;
	_UnSupported_Vehicle_End_Distance = getNumber (configFile >> "CfgVehicles" >> Typeof _towing >> "Load_End_Distance");_Edistance = _UnSupported_Vehicle_End_Distance;	
};
